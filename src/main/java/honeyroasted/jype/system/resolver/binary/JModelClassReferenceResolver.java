...
public class JModelClassReferenceResolver implements JTypeResolver<ClassModel, JType> {
    ...
    @Override
    public Optional<? extends JType> resolve(JTypeSystem system, ClassModel value) {
        ClassDesc desc = value.thisClass().asSymbol();
        JClassLocation location = JClassLocation.of(value);
        ...
        Optional<InnerClassesAttribute> innerAttr = value.findAttribute(Attributes.INNER_CLASSES);
        Optional<EnclosingMethodAttribute> enclMeth = value.findAttribute(Attributes.ENCLOSING_METHOD);
        ...
        if (sigAttr.isPresent()) {
            ...
            if (value.superclass().isPresent()) {
                Optional<? extends JType> superclass = system.resolve(JClassLocation.of(value.superclass().get()));
            ...
            for (ClassEntry inter : value.interfaces()) {
