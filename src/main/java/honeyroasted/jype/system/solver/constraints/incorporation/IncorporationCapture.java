...
public class IncorporationCapture implements ConstraintMapper.Unary<TypeConstraints.Capture> {
    ...
    public void process(PropertySet context, ConstraintNode node, TypeConstraints.Capture constraint) {
        ...
        if (left.classReference().typeEquals(right.classReference()) &&
            ...
            ConstraintTree initialBounds = new ConstraintTree(TrackedConstraint.of(Constraint.multi(ConstraintNode.Operation.AND, varMap)), ConstraintNode.Operation.AND);
            varMap.forEach(c -> initialBounds.attach(c.tracked(initialBounds.trackedConstraint()).createLeaf().overrideStatus(true)));

            node.expand(ConstraintNode.Operation.AND, left.typeSystem().operations().initialBoundsApplier().process(initialBounds));
                    ...
                    if (a instanceof WildType) {
                        for (ConstraintNode otherNode : node.neighbors(ConstraintNode.Operation.AND, ConstraintNode.Status.TRUE)) {
                            ...
                            if (other instanceof TypeConstraints.Equal eq && ((eq.left().typeEquals(alpha) && !(eq.right() instanceof MetaVarType))
                                ...
                                node.expandRoot(ConstraintNode.Operation.AND)
                                        .attach(Constraint.FALSE.tracked(node.trackedConstraint(), otherNode.trackedConstraint()).createLeaf().overrideStatus(false));
                                ...
                                if (st.left().typeEquals(alpha) && !(st.right() instanceof MetaVarType)) {
                                    ...
                                    if (a instanceof WildType.Upper wtu) {
                                        if (wtu.hasDefaultBounds()) { //?
                                            ...
                                            bi.forEach(bii -> node.expandRoot(ConstraintNode.Operation.AND)
                                                    .attach(new TypeConstraints.Subtype(theta.apply(bii), r).tracked(node.trackedConstraint(), otherNode.trackedConstraint())));
                                        } else {
                                            if (vi.hasDefaultBounds()) {
                                                //Case where A_i is a wildcard of form ? extends T, alpha_i <: R, and B_i is Object => T <: R (18.3.2 Bullets #3.2)
                                                wtu.upperBounds()
                                                        .forEach(ti -> node.expandRoot(ConstraintNode.Operation.AND)
                                                                .attach(new TypeConstraints.Subtype(ti, r).tracked(node.trackedConstraint(), otherNode.trackedConstraint())));
                                            }
                                        }
                                    ...
                                    if (a instanceof WildType.Upper wtu) {
                                        ...
                                        wtl.lowerBounds().forEach(ti -> node.expandRoot(ConstraintNode.Operation.AND)
                                                .attach(new TypeConstraints.Subtype(r, ti).tracked(node.trackedConstraint(), otherNode.trackedConstraint())));
                                    }
                                }
                            }
                        }
                        ...
                        node.expandRoot(ConstraintNode.Operation.AND)
                                .attach(new TypeConstraints.Equal(alpha, a).tracked(node.trackedConstraint()).createLeaf().overrideStatus(true));
                    }
                }
            }
        }
